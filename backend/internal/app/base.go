	...
	go func() {
		...
		for {
			...
			default:
				...
				a.Services.WorkspaceManagerService.Create(ctx, build)
				a.Services.GitService.CloneRepository(ctx, build, tempDirPath)
				a.Services.BuildService.BuildApplication(ctx, build, tempDirPath)
				...
				a.Services.DeployService.DeployApplication(ctx, build, deployment, tempDirPath)
